...
public:
  ...
  void  generate_points(std::vector<cv::Mat>* navf_points,std::vector<line_class>* lines,
    ...
    flight_lines.at(0).at(0).row(0).col(0)=navf_points->at(first_point_index).at<double>(0,0)-addittional_wp/2*advance_vector.at<double>(0,0);
    flight_lines.at(0).at(0).row(1).col(0)=navf_points->at(first_point_index).at<double>(1,0)-addittional_wp/2*advance_vector.at<double>(1,0);
    std::cout<<"x: \t\ty: "<<std::endl;
    //std::cout<<"x: "<<flight_lines.at(0).at(0).at<double>(0,0)<<"\ty: "<<flight_lines.at(0).at(0).at<double>(1,0)<<std::endl;
    ...
    for(int i=1;i<flight_lines.at(0).size();i++){
      ...
      //std::cout<<"x: "<<flight_lines.at(i).at(j).at<double>(0,0)<<"\ty: "<<flight_lines.at(i).at(j).at<double>(1,0)<<std::endl;
      std::cout<<flight_lines.at(0).at(i).at<double>(0,0)<<"\t"<<flight_lines.at(0).at(i).at<double>(1,0)<<std::endl;
    }
    ...
    for(int i=1;i<lines_number;i++){
      ...
      if(num_interc>1){
        ...
        //std::cout<<std::endl;
        //std::cout<<"x: \t\ty: "<<std::endl;
        //std::cout<<"x: "<<flight_lines.at(i).at(0).at<double>(0,0)<<"\ty: "<<flight_lines.at(i).at(0).at<double>(1,0)<<std::endl;
        ...
        for(int j=1;j<flight_lines.at(i).size();j++){
          flight_lines.at(i).at(j).row(0).col(0)=flight_lines.at(i).at(j-1).at<double>(0,0)+(pow(-1,i))*advance_vector.at<double>(0,0);
          flight_lines.at(i).at(j).row(1).col(0)=flight_lines.at(i).at(j-1).at<double>(1,0)+(pow(-1,i))*advance_vector.at<double>(1,0);
          //std::cout<<"x: "<<flight_lines.at(i).at(j).at<double>(0,0)<<"\ty: "<<flight_lines.at(i).at(j).at<double>(1,0)<<std::endl;
  ...
  uint  find_initial_point_and_side_vector(std::vector<cv::Mat>* navf_points,std::vector<line_class>* lines,
                                           cv::Mat* row_vector,double x_max,double y_max,double x_min,double y_min,
                                           cv::Mat* side_vector, double line_space,double* line_number, bool debug){
    uint  index=0;
    ...
    for(int i=0;i<num_points;i++){
      ...
      //std::cout<<"row line"<<row_line.line_<<std::endl;
      ...
      //std::cout<<"x max "<<x_max<<"x min "<<x_min<<"y max "<<y_max<<"y min "<<y_min<<std::endl;
      do{
        num_interc=0;
        /*std::cout<<"row line "<<row_line.line_<<std::endl;
        std::cout<<"interceptiones"<<std::endl;*/
        for(int j=0;j<num_points;j++){
          if(lines->at(j).find_interception(&row_line.line_)){
      ...
      do{
        ...
        for(int j=0;j<num_points;j++){
          if(lines->at(j).find_interception(&row_line.line_)){
            //std::cout<<"interception"<<lines->at(j).intercept_point<<std::endl;
            //std::cout<<"x max "<<x_max<<"x min "<<x_min<<"y max "<<y_max<<"y min "<<y_min<<std::endl;
    ...
    for(int i=1;i<num_points;i++){
      if(num_lines.at(i)>num_lines.at(i-1)){
        index=i;
        direction.at(i).copyTo(*side_vector);
      }
    }
    if(false){std::cout<<"Index: "<<index<<"\tnum lines:"<<num_lines.at(index)<<"\tside vector: \n"<<*side_vector<<std::endl;}
    *line_number=num_lines.at(index)+1;
    return  index;
  }
