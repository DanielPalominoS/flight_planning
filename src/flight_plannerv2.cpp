...
public:
  ...
  line_class(double  x1,double x2,double y1,double y2){
    ...
    line_.row(0).col(1)=x2-x1;
    line_.row(1).col(0)=y1;
  ...
  line_class(double  x1,double y1,cv::Mat vector){
    //ecuación parametrica recta columna 0 componente x; columna 1 componente y
    line_.row(0).col(0)=x1;
    line_.row(0).col(1)=vector.at<double>(0,0);
    line_.row(1).col(0)=y1;
  ...
  bool find_interception(cv::Mat* line_to_int){
    ...
    vec=line_to_int->at<double>(0,0)-line_.at<double>(0,0);
    vec=line_to_int->at<double>(0,1)-line_.at<double>(0,1);
    ...
    intercept_point.x=line_.at<double>(0,0)+line_.at<double>(0,0)*t.at<double>(0,0);
    intercept_point.y=line_.at<double>(0,1)+line_.at<double>(0,1)*t.at<double>(0,0);
    success=true;
  ...
  /*
  /// finds the interception between two lines if exists
  bool line_interception(cv::Mat* line1 ,cv::Mat* line2, double* x, double* y){
    bool  success=false;
    cv::Mat A=cv::Mat(2,2,CV_64F);
    cv::Mat vec=cv::Mat(2,1,CV_64F);
    cv::Mat t=cv::Mat(2,1,CV_64F);
    A.row(0).col(0)=line1->at<double>(1,0);
    A.row(0).col(1)=-line2->at<double>(1,0);
    A.row(1).col(0)=line1->at<double>(1,1);
    A.row(1).col(1)=-line2->at<double>(1,1);

    vec=line2->at<double>(0,0)-line1->at<double>(0,0);
    vec=line2->at<double>(0,1)-line1->at<double>(0,1);
    ...
    *x=line1->at<double>(0,0)+line1->at<double>(0,0)*t.at<double>(0,0);
    *y=line1->at<double>(0,1)+line1->at<double>(0,1)*t.at<double>(0,0);
    success=true;
    return  success;
  }*/
...
public:
  ...
  bool plannerCB(flight_planning::generate_plan::Request &req,
    ...
    std::cout<<"row vector image: "<<crop_row_vector_image<<std::endl;
    ...
    std::cout<<"row vector navf: "<<crop_row_vector_navf<<std::endl;
    ...
    std::cout<<"min x: "<<min_vector.at<double>(0,0)<<"\tmin y"<<min_vector.at<double>(1,0)<<std::endl;
    std::cout<<"max x: "<<max_vector.at<double>(0,0)<<"\tmax y"<<max_vector.at<double>(1,0)<<std::endl;

    return  true;
  }

  ///find max and min x and y from the crop contour
  ///
  /// \brief find_max_min_x_y
  /// \param points
  /// \param max_vector
  /// \param min_vector
  ///
  ...
  uint  find_initial_point(std::vector<cv::Mat>* crop_image_coords,double cam_ori, cv::Mat imu_ori, cv::Mat uav_gps_posi,
                           cv::Mat k, cv::Mat  ref_gps,bool are_degrees, double scale, double fl){
    /*(double cam_ori, cv::Mat imu_ori, cv::Mat uav_gps_posi,
                              cv::Mat k,cv::Mat image_coord, cv::Mat ref_gps_posi,
                              cv::Mat* navf_point, bool is_degrees, double scale, double fl)*/
    uint  index;
    std::vector<cv::Mat>  navf_coords;
    cv::Mat ref_navf_coor;
    geod2nf(ref_gps,ref_gps,&ref_navf_coor,are_degrees);
    std::vector<double> dist;
    navf_coords.resize(crop_image_coords->size());
    dist.resize(crop_image_coords->size());
    //std::cout<<"size "<<crop_image_coords->size()<<std::endl;
    //std::cout<<"size "<<navf_coords.size()<<std::endl;
    for(int i=0;i<crop_image_coords->size();i++){
      navf_coords.at(i)=cv::Mat(3,1,CV_64F);
      if2navf(cam_ori,imu_ori,uav_gps_posi,k,crop_image_coords->at(i),ref_gps,&navf_coords.at(i),are_degrees,scale,fl);
      dist.at(i)=calculate_distance(ref_navf_coor.at<double>(0,0),ref_navf_coor.at<double>(1,0),
                                    navf_coords.at(i).at<double>(0,0),navf_coords.at(i).at<double>(1,0));
    }

    index=0;

    for(int i=1;i<crop_image_coords->size();i++){
      if(dist.at(i)>dist.at(index)){
        index=i;
      }
    }
    ...
    std::cout<<"index "<<index<<std::endl;
    std::cout<<"initial point navf "<<navf_coords.at(index)<<std::endl;
